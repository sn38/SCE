...
slider.oninput = function() {
  ...
  while(pourcentage==null){
	...
	if(position<10){ //Ajouter un 0 devant la position pour les heures < 10 (4h devient 04h)
  	pourcentage = $("#interval_0"+position).data("pourcentage");
	}
	else{
	...
	position = position - 1; //descendre positon en dessous
  }
  ...
  label.innerHTML = "0h à "+this.value+"h";
	...
	for(i;i<parseInt(slider2.value);i++){
		if(i<10){
			valeur = $("#casse_0"+i).data("percentage");
		}
		else{
	...
	if(max>=maximum){
		...
		label2.innerHTML = max+"h à "+slider2.value+"h";
		...
		parseInt(minimum_du3);
		//Ne pas dépasser 24h 
		if(minimum_du3<24){
			//minimum_du3++;
			}
		output2.innerHTML = slider2.value+"h"; // Afficher valeur
	...
	for(j;j<24;j++){
		...
		if(j<10){
		valeur = $("#casse_0"+j).data("percentage");
		}
		else{
