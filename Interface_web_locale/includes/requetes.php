...
<!--Grouper les valeurs de chaque heure-->
<?php
$ma_requette="SELECT SUM(watts) as watt, heure as heure FROM table_stockage GROUP BY date_format( heure, '%H' );"; 
$rep =  $connexion->prepare($ma_requette);
$rep->execute(array($ma_requette));
$somme_pourcent = 0;
...
while ($val = $rep->fetch())
    {
        //Calcul pourcentage
        $valeur = round(($val['watt']/$total)*100,2);
        //Format avec que l'heure
        $heure = DateTime::createFromFormat("Y-m-d H:i:s", $val['heure']);
        $heure = $heure->format('H'); 
        //Somme des pourcentages à chaque heure
        $somme_pourcent += $valeur;
        ?>  
        <!--Stockage dans les attributs-->
        <div  class="valeur_heure" id = "interval_<?= $heure;?>" data-pourcentage="<?= $somme_pourcent;?>" data-heure="<?= $heure;?>"></div>
        <div  class="casse_heure" id = "casse_<?= $heure;?>" data-percentage="<?= $valeur;?>" data-heure="<?= $heure;?>"></div>
        <?php
