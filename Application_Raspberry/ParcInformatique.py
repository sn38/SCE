...
class ParcInformatique:
    ...
    def lire_donnee(self, local, intervalle):
        ...
        data = self.bdd.read_data(local, int(nb_val))   #Les donnees lues

        # si c'est Null on attribut des donnees manuellement
        if (data[0][0] == None):
            data = [(0, 0, '0000-00-00 00:00:00', 0, 0)]
        # si on a lue des données
        else:
            # si la periode de lecture est inferieur a 1 jour on lit normalement
            if (intervalle <= 86400):
            ...
            if (intervalle > 86400):
                ...
                for k in range(repetition):
                    data = self.bdd.read_data(local, int(nb_val))   #Je lis
